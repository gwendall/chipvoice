...
const capture=captureNsf(bytes,{track:nativeSources.zelda.track,frames:6500});
assertZeldaOverworld(capture);
...
for(let i=0;i<1920;i++)assert.equal(signature(373+i),signature(2293+i),`Loop repeat frame ${i}`);
const {score,native}=nsfPerformance(capture,{title:'Zelda · Overworld',endFrame:2293,loopFrame:373,source});

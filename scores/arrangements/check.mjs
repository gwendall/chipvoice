...
export async function checkArrangements(){
  ...
  for(const id of arrangementIds){
    ...
    if(id==='mario'){
      ...
      evidence={musicCommands:commands.length,exactCycles:true,oracle:reference.oracle};
    }else evidence=comparePerformance(score,reference);
    const ports={};
    for(const chip of arrangementChips){
      const plan=planPerformance(score,chip,{allowLoss:true});
      ...
      ports[chip.spec.id]={played:plan.notes.length,omitted:missing.length};
    }

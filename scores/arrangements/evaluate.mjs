...
for(const id of arrangementIds){
 ...
 for(const chip of arrangementChips){
  ...
  const row={chip:chip.spec.id,seconds:plan.seconds,loopStartSeconds:plan.loopStartSeconds,mode:id==='mario'&&chip.spec.id==='2a03'?'native-commands':'adaptation',notes:plan.notes.length,losses:plan.losses,repeat,mixer,asset:await asset(`${id}-${chip.spec.id}`,audio)};
 ...
 if(id==='mario'){
  ...
  piece.reference={kind:'independent-nsf',title:'Independent NSF emulator',evidence:JSON.parse(await readFile('scores/arrangements/references/mario.json')),asset:await asset('mario-reference',{left,right,sampleRate:44100})};
 }

...
import {prepareFmTimbres} from '../analyze-fm-timbres.mjs';
...
const percussion=extractDacPercussion(bytes);
const score=prepareFmTimbres(vgmPerformance(importVgm(bytes),{title:'Sonic · Green Hill Zone',source,dacPercussion:percussion.notes}));
await mkdir(out,{recursive:true});await writeFile(`${out}/dac-analysis.json`,JSON.stringify(percussion,null,2));await writeFile(`${out}/sonic.json`,JSON.stringify(score)+'\n');await writeFile(`${out}/sonic-native.vgm`,bytes);

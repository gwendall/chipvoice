...
- `core.ts` handles locale paths and translation. Compiled templates also translate canonical messages produced by workers and the SDK at the presentation boundary. This keeps locale out of performance documents, render caches and audio-effect dependencies. The bounded translation cache is unrelated to the audio loop.
- `[locale]` is the single Next.js route tree. Both locales are pre-rendered. `proxy.ts` rewrites English URLs internally to `/en`; `/en` itself redirects to the original canonical address. APIs, audio, downloads and machine-readable resources retain their stable URLs.
- `I18nProvider` supplies the selected dictionary. The selector loads the other dictionary on demand and updates native history so the instrument is not unmounted. Back/forward restores language. Internal page links use the locale-aware `Link`.

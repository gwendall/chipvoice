...
<a id="artist-and-agent-lifecycle-2026-09-08"></a>
### アーティストとエージェントのライフサイクル（2026-09-08）

実装済み：所有者に属する独立したアーティスト、メール不要で有効期限と権限を限定したエージェント認証、所有者による取り消し、公開前の HTTP 評価、固定 MP3 と既存 WAV からの変換、コンソール別グループ化、オリジナルのピクセルアバター編集、英語・日本語の UI と文書。[仕様と検証](ARTISTS-AND-AGENTS_ja.md)を参照してください。
...
- ページ移動中も再生を保持し、明示的な操作と準備完了後に音源を交代。
- モバイル・デスクトップの固定プレーヤー、再生位置、先頭、リピート、音量。
- 一覧・ライブラリ・アーティストのカードから直接再生し、次の曲へ進む。
- 保存済みの公開音声と作者を保持。ブラインド比較の識別情報は移動後も非表示。
- 同じループ楽譜を作曲画面に引き継ぎ、パッド・録音のライブ音源は維持。
- 位置とピアノロールの更新で編集画面全体を再描画しない。
- 英語・日本語、部品一覧、ブラウザー・音声・セッションの回帰検証。

仕様と検証：[継続再生](CONTINUOUS-PLAYBACK-LAB_ja.md)。
再読み込み後の復元、複数タブ間の調整、共同編集は別の作業です。


## 操作から再生までの遅延 — 2026-09-08 の監査

状態：原因調査済み、実装は未着手。[測定・試作・合格条件](INTERACTION-LATENCY_ja.md)を参照。

- [ ] LAT-1 — 準備済み選択の即時処理、楽譜取得の重複除去、メタデータ更新の独立化。
- [ ] LAT-2 — 参照音声の遅延取得、音楽設定をキーにした上限付きキャッシュ。
- [ ] LAT-3 — 既存コンパイラーと DSP による段階的プレビューを WAV 書き出しから分離。
- [ ] LAT-4 — 音楽内容の差分更新、互換エンジンの再利用、準備状態のイベント通知。
- [ ] LAT-5 — 機種別の状態保存能力に基づく、曲途中の切り替えとシーク。
- [ ] LAT-6 — 公開曲情報の再利用、部分読み出しによる音声の範囲配信。
- [ ] LAT-7 — 遅延予算、継続レンダリング、音声連続性の回帰テスト。

試作の先頭区間はオフライン結果と一致したが、リアルタイムプレーヤーの実装完了を意味しない。既存の独立参照音源との忠実性と連続再生の契約を維持する。

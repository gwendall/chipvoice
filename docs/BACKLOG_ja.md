...
逐次再生の実装とデスクトップ・モバイルのブラウザ検証は完了した。任意のレジスタ履歴をその場でリタイミングする API は今回の対象外。不要な worklet は破棄し、音量ゼロの裏で DSP を動かし続けない。未準備の履歴再構築や機器・通信の遅延は残る。詳細は [操作遅延の実装と検証](INTERACTION-LATENCY_ja.md) を参照。

...
SNES の自動アダプテーションでは、内部の加算とエコー帰還の前に全音声共通の
音量余裕を確保します。FIR 係数の絶対値の和 172/128、帰還 56/128、整数音量、
最大 80 ms のリリースと 12 ms の書き込みずれを考慮し、共通予算を 38 に
制限します。30 ms のランプとレジスタ保持区間の最小ゲインで先に調整します。
弱い音が最小音量ステップ未満になった場合は `mix-bus-resolution` を報告します。
原作コマンドの再生と `mix:false` には適用しません。ワークレットに新しい処理は
追加しません。この保護は一緒に準備した音符が対象です。前のフレーズの
リリースが残る場合は、音符をスケジュールする前に、連続フレーズを共通の準備区間にまとめる必要があります。

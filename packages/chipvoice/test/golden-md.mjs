...
/**
 * The Mega Drive render, locked: the golden score arranged for the third
 * chip, hashed and compared with the hash on file. Everything `golden.mjs`
 * says applies; the file is written on the first run.
 */
const FILE = new URL('./golden-md.json', import.meta.url);
...
const result = renderSong(arrange(SCORE, 'md'), { seconds: SECONDS, chip: 'md', stereo: true });
...
if (process.argv.includes('--update') || !existsSync(FILE)) {
  ...
  console.log(`golden-md.json written: ${sha256.slice(0, 16)}  rms ${current.rms}  peak ${current.peak}`);
  process.exit(0);
}

const expected = JSON.parse(readFileSync(FILE, 'utf8'));
const ok = expected.sha256 === sha256;
console.log(`${ok ? 'PASS' : 'FAIL'}  the Mega Drive render is byte for byte what it was  ${sha256.slice(0, 16)}${ok ? '' : `, expected ${expected.sha256.slice(0, 16)}`}  rms ${current.rms} (was ${expected.rms})  peak ${current.peak} (was ${expected.peak})`);

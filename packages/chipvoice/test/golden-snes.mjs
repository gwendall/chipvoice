 ...
 */
const FILE = new URL('./golden-snes.json', import.meta.url);
...
const result = renderSong(arrange(SCORE, 'snes'), { seconds: SECONDS, chip: 'snes', stereo: true });
...
if (process.argv.includes('--update') || !existsSync(FILE)) {
  ...
  console.log(`golden-snes.json written: ${sha256.slice(0, 16)}  rms ${current.rms}  peak ${current.peak}`);
  process.exit(0);
}

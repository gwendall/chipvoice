 ...
 *
 * Version 1.61 of the format, the one that added the NES APU. Register writes
 * are `0xB4 aa dd` with `aa` the low byte of the address. The header carries
 * the chip's clock and the total length; a GD3 tag at the end carries the
 * names, because that is what a player shows.
 */
...
/**
 * @param events register writes, stamped in CPU cycles of the 2A03
...
export function toVgm(events: RegisterEvent[], cycles: number, options: VgmOptions = {}): Uint8Array {
  const samples = (cycle: number) => Math.round((cycle * SAMPLE_RATE) / CPU_HZ);
  const total = samples(cycles);
  const writes = events
    .filter((e) => e.at < cycles && e.addr >= 0x4000 && e.addr <= 0x401f)
    .map((e) => ({ at: samples(e.at), addr: e.addr & 0xff, value: e.value & 0xff }))
    .sort((a, b) => a.at - b.at);
  ...
  view.setUint32(0x84, CPU_HZ, true);
 ...
 */
function tag(options: VgmOptions): Uint8Array {
  const strings = [
    options.title ?? "",
    "",
    options.game ?? "",

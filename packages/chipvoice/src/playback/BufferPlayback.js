...
export class BufferPlayback {
    ...
    try {
      const [buffers, resolvedPresentation] = await Promise.all([Promise.all(
        entries.map(async (entry, index) => {
          ...
          if (!response.ok)
            throw new Error(
              `Audio unavailable (${response.status}). Try again.`,
            );
          ...
          if (entry.loopFadeSeconds) {
            const start = Math.max(
              0,
              Math.round((entry.loopStartSeconds ?? 0) * buffer.sampleRate),
            );
            const size = Math.max(
              0,
              Math.min(
                Math.round(entry.loopFadeSeconds * buffer.sampleRate),
                Math.floor((buffer.length - start) / 2),
              ),
            );
            for (
              let channel = 0;
              ...
              for (let i = 0; i < size; i++) {
                const gain = size > 1 ? i / (size - 1) : 0;
                samples[start + i] *= gain;
                samples[buffer.length - 1 - i] *= gain;
              }
            }
          }
  ...
  clockGroup(at) {
    ...
    if (at < group.at) {
      for (let i = this.timeline.length - 1; i >= 0; i--)
        if (this.timeline[i].at <= at) {
          group = this.timeline[i];
          break;
        }
    }
    return group;
  ...
  phase(at = outputTime(this.context)) {
    ...
    if (position >= group.duration)
      position = group.loop
        ? group.loopStart +
          ((position - group.loopStart) % (group.duration - group.loopStart))
        : group.duration;
  ...
  setLoop(loop) {
    ...
    if (this.group) {
      ...
      if (group.ended && loop) {
        group.loop = true;
        this.swap(group.loopStart / group.duration);
        this.changed();
        return;
      }
      ...
      group.offset = this.phase(at) * group.duration;
      group.at = at;
      group.loop = loop;
      this.remember(group);
  ...
  swap(requestedPhase) {
    ...
    if (this.retiring) {
      this.pendingPhase =
        requestedPhase ?? this.phase(this.context.currentTime);
      return;
    }
    ...
    const parts = this.buffers.flatMap((buffer, index) => {
      ...
      const level = new Fade(
        this.context,
        fade.node,
        index === this.side ? this.levels[index] * this.volume : 0,
      );
    ...
    fade.toValue(1, at);
    previous?.fade.toValue(0, at);
    if (previous) {
      this.retiring = previous;
      // Sources stop on the audio clock even when the main thread is busy.
      for (const part of previous.parts) part.source.stop(at + 0.06);
      this.transition = new Promise((resolve) => {
        this.finishTransition = resolve;
        const cleanup = () => {
          if (!this.disposed && this.context.currentTime < at + 0.06) {
            this.timer = setTimeout(cleanup, 30);
            return;
          }
  ...
  finishAtEnd(group) {
    ...
    const remaining =
      group.at + group.duration - group.offset - outputTime(this.context);
    if (remaining > 0) {
      this.endTimer = setTimeout(
        () => this.finishAtEnd(group),
        Math.max(20, remaining * 1000),
      );
      return;
    }
    this.offset = 1;
  ...
  pause() {
    ...
    if (
      group &&
      (group.ended || (!group.loop && group.offset >= group.duration))
    )
      this.release(group);
    else if (group) {
      group.fade.toValue(0, this.context.currentTime, 0.025);
      for (const part of group.parts)
        part.source.stop(this.context.currentTime + 0.03);
      if (!this.retiring) {
        this.retiring = group;
        this.transition = new Promise((resolve) => {
          this.finishTransition = resolve;
          group.parts[0].source.onended = () => {
            ...
            const pending = this.pendingPhase;
            this.pendingPhase = null;
            if (!this.disposed && this.playing && pending !== null)
              this.swap(pending);
            resolve();
          };
        });

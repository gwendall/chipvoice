...
import {noteToFreq, type PlayNoteOptions} from './driver.js';
...
export function prepareMixPhrase(chip: ChipDefinition, notes: readonly MixPhraseNote[], options: MixOptions = {}) {
  ...
  const voiced = notes.map((note, index) => {
    ...
    if (notes.some((other, otherIndex) => otherIndex !== index && other.voice === note.voice && other.at < note.at + note.duration && note.at < other.at + other.duration)) throw new Error('Mix phrase contains overlapping hardware voices');
  ...
  const prepared = notes.map((note, index) => {
    const volume = [];
    const count = Math.max(1, Math.round(note.duration * 60));
    for (let frame = 0; frame < count; frame++) {
      ...
      volume.push(policy.level(index, control, note.at + frame / 60, typeof note.note === 'number' ? note.note : 9));
    }

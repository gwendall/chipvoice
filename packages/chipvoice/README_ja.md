...
[API と検証範囲](https://github.com/gwendall/chipvoice/blob/main/docs/MIXING-API_ja.md)。
...
MIDI の役割はトラックの順番ではなく音符から推定し、Web プレイヤーで確認・
変更できます。元から無音の音符は音声を占有せず、SNES の内部加算には
共通の音量余裕を確保します。楽曲全体、ダイナミクス、校正誤差を検証しますが、
決定論的な出力は原作の音色や主観的なミックス品質を保証するものではありません。

0.16.3 から SNES の標準音色にはエコーを加えません。ネイティブ FM の移植では、
ローカルな音色 ID を MIDI 楽器番号と解釈せず、実測した音高と音量変化を使います。

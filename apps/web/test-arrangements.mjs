...
try{
 ...
 await page.getByRole('button',{name:'Independent original reference',exact:true}).click();assert.ok(await audible(page)>.001,'independent NSF renderer is audible');
 ...
 await page.getByRole('button',{name:'Sonic 8 parts'}).click();await page.waitForFunction(()=>!document.querySelector('.arrangement-versions button').disabled,{},{timeout:60000});
 assert.equal(await page.locator('.arrangement-parts button').count(),9,'all eight native source parts are selectable');
 ...
 assert.ok(await audible(page)>.001,'imported polyphony rendered locally');
 await page.getByLabel('Import MIDI',{exact:true}).setInputFiles({name:'invalid.mid',mimeType:'audio/midi',buffer:Buffer.from('broken MIDI')});
 await page.getByRole('alert').filter({hasText:'Choose another MIDI'}).waitFor();
 await page.waitForFunction(()=>!document.querySelector('.arrangement-versions button').disabled,{},{timeout:60000});
 assert.match(await page.getByRole('alert').filter({hasText:'Choose another MIDI'}).textContent(),/Choose another MIDI/,'parse errors persist after the previous audio is restored');
 assert.ok(await audible(page)>.001,'invalid import retains audible previous music');
 ...
 await writeFile(new URL('result.json',out),JSON.stringify({pass:true,errors,checks:['lazy assets','native/reference audio','continuous four-console selection','8 native source parts','local polyphonic MIDI','persistent invalid import feedback','tempo and transpose','Stop wins','320/390/768 typography and overflow']},null,2));

...
assert.equal(t('7 notes omitted.'),'7 個の音符を省略。');assert.equal(t('Channel 12: controller 10 is not reproduced'),'チャンネル 12：コントローラー 10 は再現していません');
...
const covered=text=>Object.hasOwn(en,text.trim())||t(text)!==text;
...
async function walk(dir){for(const entry of await readdir(dir,{withFileTypes:true})){const path=`${dir}/${entry.name}`;if(entry.isDirectory())await walk(path);else if(path.endsWith('.tsx')&&!path.includes('/card/')){
 ...
 if(!inCode&&ts.isJsxText(node)&&/[A-Za-z]/.test(node.text))check(node.text);

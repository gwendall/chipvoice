 ...
 return <><SiteHeader active="lab"/><main className="demo-main lab-main">
  ...
  {selection&&display&&<>
   <section className="console lab-console" aria-label={t("Listening lab")}>
    ...
    <DisplayPanel className="lab-display"><div className="screen-title"><div><span className="screen-kicker">{(hidden?t('IDENTITIES HIDDEN'):t("{v0} / {v1}",{v0:t(display.row.chip.toUpperCase()),v1:t(t(roles[display.role]).toUpperCase())}))}</span><h2>{t(display.row.title)}</h2></div><span className="lab-loop">{t(display.row.seconds.toFixed(2))}{t(" SEC LOOP")}</span></div>

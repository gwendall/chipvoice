...
export default function Lab() {
 ...
 const [caseId,setCaseId]=useState('overworld-snes'),[role,setRole]=useState('mix'),[reference,setReference]=useState('baseline');
 ...
 const selection=useMemo<Selection|null>(()=>{
  ...
  const selectedRole=row.assets[role]?role:'mix';
 ...
 return <><SiteHeader active="lab"/><main className="demo-main lab-main">
  <div className="lab-intro"><div><span className="micro">THE LISTENING ROOM</span><h1>Same notes.<br/><span>Different machines.</span></h1><p>Find the sound you love. Pull a tune apart, switch consoles and hear what changes.</p></div><div className="lab-intro-note"><span className="status-light active"/> FIVE MACHINES / ONE SCORE<br/><span>Press play once. Keep exploring.</span></div></div>
  ...
  {selection&&display&&<>
   ...
   {display.row.source&&<details className="cartridge-source"><summary>About this arrangement · credits & source</summary><p>{display.row.adaptation}</p><p>Music by {display.row.composer}. Reference: {display.row.source.transcriber}. {display.row.source.excerpt}.</p><a href={display.row.source.url} target="_blank" rel="noreferrer">View the source partition ↗</a></details>}

...
export default function PromptComposer({ target }: { target: string }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/v1/profiles", { signal: abort.signal }).then(async r => {
      if (r.ok) { const data = await r.json(); setArtists(data.items); setArtist(current => current || data.items[0]?.id || ""); }
    }).catch(() => {});
    return () => abort.abort();
  }, [open]);
  useEffect(() => {
  ...
  return <details className="prompt-composer" open={open} onToggle={e => setOpen(e.currentTarget.open)}>
    ...
    <form onSubmit={e => { e.preventDefault(); void generate(); }}>
      ...
      <div className="project-actions"><Button type="submit" disabled={busy || !prompt.trim()}>{t("Generate music")}</Button>{busy && job && <Button type="button" onClick={() => void cancel()}>{t("Cancel composition")}</Button>}</div>
    </form>

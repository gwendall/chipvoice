...
import {playbackFor, playbackSession, releasePlayback} from '@/player/session';
import {projectFromPerformance} from 'chipvoice';
import {openProjectDraft} from '@/create/drafts';
...
export default function Arrangements({catalogue,initialOverview,active=true,embedded=false}:{catalogue?:Report;initialOverview?:Overview;active?:boolean;embedded?:boolean}){
 ...
 useEffect(()=>{
  ...
  const timer=setTimeout(()=>{void(async()=>{
   ...
   if(row&&part==='mix'&&tempo===100&&transpose===0){
    ...
    let native:PerformancePlan|undefined,nativeVgm:ArrayBuffer|undefined;
    if(piece.native?.chip===chip&&tempo===100&&transpose===0){
     if(piece.native.format==='vgm'){const response=await fetch(piece.native.file,{signal:abort.signal});if(!response.ok)throw Error('Source arrangement unavailable');nativeVgm=await response.arrayBuffer();}
 ...
 return <>{!embedded&&<SiteHeader/>}<main className="demo-main arrangements-main">
  ...
  {piece&&<section className="console arrangement-deck" aria-label={t("Complete arrangement player")}>
   ...
   <DisplayPanel><div className="screen-title"><div><span className="screen-kicker">{t(loaded?.chip.toUpperCase()??chip.toUpperCase())} / {(loaded?.part==='mix'||!loaded?t('FULL MIX'):t('ISOLATED PART'))}</span><h2>{(loaded?.pieceId??pieceId)==='imported'?(loaded?.title??piece.title):t(loaded?.title??piece.title)}</h2></div><span>{(loaded||current||preparation?.seconds?t("{v0} SEC",{v0:t((loaded?.seconds??current?.seconds??preparation!.seconds!).toFixed(1))}):t('AUDIO PENDING'))}</span></div>
    ...
    {pending&&preparation?<section className="arrangement-preparation" aria-label={t("Audio preparation")} aria-busy="true">
     ...
     <div className="render-progress-caption"><span>{(preparation.percent===undefined?t('Working…'):t("{v0}% rendered",{v0:preparation.percent}))}</span><span>{Math.max(0,Math.floor((now-preparation.startedAt)/1000))}{t(" s elapsed")}</span></div>
     <p>{(loaded&&audio.playing?t('The current music keeps playing.'):(audio.playing?t('Playback starts automatically when ready.'):t('Playback is paused. Press Play to listen when ready.')))}</p>
    </section>:<p>{(audio.playing?t('Playing continuously · explore another sound.'):t('Press Play or tap a console to start.'))}</p>}
  ...
  {(error||audio.error)&&<p className="ui-status ui-error" role="alert">{errorText(error||audio.error)} <Button onClick={()=>setSession(s=>s+1)}>{t("Retry rendering")}</Button></p>}
  {piece&&<div className="arrangement-evidence"><section><span className="micro">{t("WHAT IS VERIFIED?")}</span><h2>{(piece.source.kind==='native'?t('The original commands.'):t('Every source part.'))}</h2><p>{(piece.source.kind==='native'?t('On its original console, each song replays the original chip commands, verified against an independent emulator. NES timing is checked in CPU cycles; Mega Drive timing follows the VGM capture. Other consoles and edited versions are adaptations.'):t('Notes, velocities, programs and exact MIDI timing are preserved in the source arrangement. This is a complete transcription; its instruments have not been verified against the original game.'))}</p><p>{t(piece.source.description??'Your MIDI stays in this browser. No file is uploaded.')}</p>{piece.source.url&&<a href={piece.source.url} target="_blank" rel="noreferrer">{t("Source & credits ↗")}</a>}</section>
   <section><span className="micro">{t("THE COST OF A PORT")}</span><h2>{(pending?t('Checking this adaptation…'):(omitted?(omitted===1?t("1 note omitted."):t("{v0} notes omitted.",{v0:omitted})):t('No voice omissions.')))}</h2><p>{(pending?t('Voice limits and instrument choices will appear when preparation finishes.'):(omitted?t('This console has fewer available voices. Higher-priority melody and bass parts reserve them first. The source is kept intact; the omissions belong to this adaptation.'):t('No extra bass, chords or drums are invented. Only parts present in the source are played.')))}</p>{!pending&&<details><summary>{t("Instrument choices & limitations")}</summary><p>{t("Adaptations balance parts using measured instrument responses. Original commands keep their native mix. Hardware limits and uncalibrated sounds are listed below.")}</p><ul>{[...new Set(losses.filter(l=>l.kind!=='voice-omitted').map(l=>l.detail)),...piece.notices].map(text=><li key={text}>{t.source(text)}</li>)}</ul></details>}<p>{t("Recordings compare at matched levels. MIDI imports and edits render locally in a worker. Audio comparisons are listening evidence, not a universal fidelity score.")}</p></section>
  </div>}
  <div className="arrangement-links">{imported&&pieceId==='imported'&&<Link href="/create" onClick={e=>{try{const project=projectFromPerformance(imported,chip);project.settings={chip,allowLoss:true,tempoScale:tempo/100,transpose};openProjectDraft(project);}catch{e.preventDefault();}}}>{t("Remix this song")} →</Link>}{pieceId!=='imported'&&<Link href={`/create?piece=${pieceId}`}>{t("Remix this song")} →</Link>}<Link href="/docs">{t("Use the API")} ↗</Link><Link href="/lab">{t("Engine listening tests →")}</Link>{pieceId!=='imported'&&<a href={`/arrangement-data/${pieceId}.json`} download>{t("Source arrangement JSON ↓")}</a>}<a href="/arrangement-data/report.json" download>{t("Evidence & porting reports ↓")}</a><a href="https://github.com/gwendall/chipvoice/blob/main/scores/arrangements/README.md">{t("Reproduce the method ↗")}</a></div>

...
export default function Arrangements({catalogue,initialOverview,active=true,embedded=false}:{catalogue?:Report;initialOverview?:Overview;active?:boolean;embedded?:boolean}){
 ...
 return <>{!embedded&&<SiteHeader/>}<main className="demo-main arrangements-main">
  <section className="arrangement-intro"><div><span className="micro">OPEN-SOURCE SOUND-CHIP EMULATION</span><h1>Old consoles.<br/><span>New JavaScript.</span></h1></div><p>We rebuilt their sound chips in JavaScript. Hear Mario, Zelda and Sonic with all their source parts. Switch consoles, follow the score, or bring your own MIDI.</p></section>
  ...
  {piece&&<section className="console arrangement-deck" aria-label="Complete arrangement player">
   <div className="console-top"><span className="micro">CHIPVOICE / FULL ARRANGEMENTS</span></div>
   ...
   <DisplayPanel><div className="screen-title"><div><span className="screen-kicker">{loaded?.chip.toUpperCase()??chip.toUpperCase()} / {loaded?.part==='mix'||!loaded?'FULL MIX':'ISOLATED PART'}</span><h2>{loaded?.title??piece.title}</h2></div><span>{loaded||current||preparation?.seconds?`${(loaded?.seconds??current?.seconds??preparation!.seconds!).toFixed(1)} SEC`:'AUDIO PENDING'}</span></div>
    <Transport playControl={<PlayButton pause playing={audio.playing} loading={pending} onClick={toggle}/>} player={player.current} overview={loaded?.overview??overview} seconds={loaded?.seconds??current?.seconds??overview?.seconds??0} part={loaded?.part??'mix'} pending={pending} active={active} onLoop={loop=>{ensure().setLoop(loop);interact();}}/>
   ...
   <div className="arrangement-controls"><RangeControl id="arrangement-tempo" label="Tempo" unit="%" min={40} max={200} value={tempo} onChange={value=>{change(()=>setTempo(value));interact();}}/><RangeControl id="arrangement-transpose" label="Transpose" unit="st" min={-24} max={24} value={transpose} onChange={value=>{change(()=>setTranspose(value));interact();}}/></div>
   ...
   <div className="arrangement-parts" aria-label="Isolate instruments"><button aria-pressed={part==='mix'} onClick={()=>{change(()=>setPart('mix'));interact();}}>Full mix</button>{piece.parts.filter(p=>p.notes>0).map(p=><button key={p.id} aria-pressed={part===p.id} onClick={()=>{change(()=>setPart(p.id));interact();}}><strong>{p.name}</strong><span>{p.id.match(/-ch-(\d+)$/)?`Ch. ${p.id.match(/-ch-(\d+)$/)![1]} · `:''}{p.notes} source notes</span></button>)}</div>
  </section>}
  ...
  <div className="arrangement-links"><Link href="/lab">Engine listening tests →</Link>{pieceId!=='imported'&&<a href={`/arrangement-data/${pieceId}.json`} download>Source arrangement JSON ↓</a>}<a href="/arrangement-data/report.json" download>Evidence & porting reports ↓</a><a href="https://github.com/gwendall/chipvoice/blob/main/scores/arrangements/README.md">Reproduce the method ↗</a></div>

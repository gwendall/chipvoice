...
onmessage=async ({data}:{data:{id:number;planOnly?:boolean;renderPlan?:PerformancePlan;importOnly?:boolean;score?:Performance;midi?:ArrayBuffer;title?:string;native?:PerformancePlan;nativeVgm?:ArrayBuffer;chip:keyof typeof chips;tempo:number;transpose:number;part:string}})=>{
 try{
  if(data.renderPlan){const audio=renderPerformance(data.renderPlan,chips[data.chip]);const wav=toWav(audio);postMessage({id:data.id,wav:wav.buffer},{transfer:[wav.buffer]});return;}
  ...
  if(data.planOnly){plan={...plan,memory:plan.memory.map(block=>({...block,bytes:new Uint8Array(block.bytes)}))};postMessage({id:data.id,plan,overview:scoreOverview(score,performanceClock(score),plan.losses,plan.silentNotes),seconds:plan.seconds,loopStartSeconds:plan.loopStartSeconds,losses:plan.losses});return;}
  let reported=-1;

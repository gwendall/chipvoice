 ...
 const t = useT();
  ...
  return <section className="composition-controls" aria-label={t("Shape your loop")}>
    ...
    <div className="composition-sliders">
      ...
      <div><RangeControl id="drum-activity" label={t("Drum activity")} unit="%" min={0} max={100} value={hasDrums ? current.drums : 0} disabled={disabled || !hasDrums} onChange={(drums,group)=>change({drums},group)}/><p>{(hasDrums?t('Give the groove more space. 100% brings every hit back.'):t('No drums in this melody. Nothing is added automatically.'))}</p></div>

...
export function Variations({ song, disabled, onEdit, onNotice }: {
 ...
 const t = useT();
  ...
  return <section className="variations" aria-label={t("Musical variations")}>
    ...
    <div className="variation-locks">{ROLES.map(role => <button key={role} className="small-button" disabled={disabled} aria-label={t(`Lock ${ROLE_NAMES[role]}`)} aria-pressed={locked.includes(role)} onClick={() => setLocked(previous => previous.includes(role) ? previous.filter(r => r !== role) : [...previous, role])}><span aria-hidden="true">{t(locked.includes(role) ? '■' : '□')}</span> {t(ROLE_NAMES[role])}</button>)}</div>

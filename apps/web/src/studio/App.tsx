  ...
  return <div className="demo-page">
    ...
    <main className="demo-main">
      ...
      <section className="console" aria-label={t("Chipvoice musical console")} onClick={event => {
        ...
        <div className="tune-heading"><div><span className="micro">{t("01 / PICK A MELODY")}</span><span className="start-hint">{(audio.playing?t('Switch sounds. Keep the melody going.'):(audio.hasInteracted?t('Press Play when you’re ready.'):t('Tap a melody or console to start the sound.')))}</span></div></div><PlayerControls player={audio.player} onPlay={togglePlayback} loading={audio.loading}/>
        ...
        <div className="transport-row">
          ...
          <button className="edit-toggle" disabled={recordLocked} aria-expanded={editing} onClick={() => setEditing(!editing)}>{(editing?t('Close editor'):t('Edit loop'))} <span aria-hidden="true">{(editing?t('−'):t('+'))}</span></button>
        </div>
      ...
      <section className="arrangement-details" aria-label={t("Arrangement sources")}>
        {sourcePreset?.source&&<details className="cartridge-source"><summary>{t("About this arrangement · credits & source")}</summary><p>{t(sourcePreset.adaptation)}</p>{sourcePreset.fidelity?.pass&&<p className="source-check">{(matchesSource?t("{v0} source notes checked · pitches, rhythm and rests · all five machines",{v0:sourcePreset.fidelity.referenceNotes}):t('Edited version · source checks apply to the original cartridge.'))}</p>}<p>{t("Music by ")}{t(sourcePreset.composer)}{t(". Reference transcription: ")}{t(sourcePreset.source.transcriber)}. {t(sourcePreset.source.excerpt)}.</p><a href={sourcePreset.source.url} target="_blank" rel="noreferrer">{t("View the source transcription ↗")}</a></details>}
      </section>
      <CompositionControls song={doc.song} disabled={recordLocked || !doc.ready} onEdit={(song,group)=>{doc.edit(song,group);measure('edit');}}/>
      <section className="keyboard-section" aria-label={t("Play notes")}><div className="keyboard-heading"><div><span className="micro">{t("PLAY A LITTLE")}</span><h2>{t("Your turn.")}</h2></div><div className="keyboard-options"><label className="sr-only" htmlFor="audition-role">{t("Audition role")}</label><select id="audition-role" value={role} onChange={e => setRole(e.target.value as Role)}>{ROLES.map(r => <option key={r} value={r}>{t(ROLE_NAMES[r])}</option>)}</select><label className="sr-only" htmlFor="musical-key">{t("Musical key")}</label><select id="musical-key" value={musicalKey} onChange={e => setMusicalKey(e.target.value)}><option value="C">{t("C major")}</option><option value="Bb">{t("B♭ major")}</option><option value="A">{t("A minor")}</option><option value="E">{t("E minor")}</option></select><button className="small-button" aria-pressed={chromatic} onClick={() => setChromatic(!chromatic)}>{t("Chromatic")}</button></div></div><div className="recording-controls"><button className="record-button" disabled={recordStarting} aria-pressed={recording} onClick={() => void toggleRecording()}><span aria-hidden="true">{(recording?t('■'):t('●'))}</span> {(recordStarting?t('Starting…'):(recording?t('Finish take'):t('Record notes')))}</button>{!recordLocked && doc.group?.startsWith('take:') && <button className="small-button" onClick={undo}>{t("Undo take")}</button>}<span className="take-status" role="status">{(recording?t("{v0} · 1/{v1}",{v0:(audio.position?t("{v0} {v1} captured",{v0:recorded,v1:(recorded === 1?t('tap'):t('taps'))}):t('Waiting for the first beat…')),v1:(doc.song.stepsPerBeat ?? 4) * 4}):t('Overdub notes and drums · one Undo per take'))}</span></div><div className={`note-keys ${role}`}>{notes.map((note, i) => <button key={note} onPointerDown={e => { if (e.button === 0) playNote(role, note); }} onClick={e => { if (e.detail === 0) playNote(role, note); }} aria-label={t("Play {v0}",{v0:t(note)})}><span>{(role === 'perc'?t(({ K: 'Kick', S: 'Snare', H: 'Hat', O: 'Open' }[note])):t(note))}</span>{keyboard[i] && <kbd>{t(keyboard[i])}</kbd>}</button>)}</div><p className="keyboard-hint">{(recording?t('Taps snap to the nearest step. Finish to hear the new loop. Untouched steps stay as they were.'):t('Use the keys or tap a note. You’re playing on the same chip as the music.'))}</p></section>

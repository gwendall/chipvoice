...
export function Editor({ song, onEdit, role, onRole, onPreview, chromatic, musicalKey, undo, redo, canUndo, canRedo }: {
 ...
 const errorText = useErrorText();
  ...
  return <section className="editor" aria-label={t("Loop editor")}>
    <div className="section-heading"><div><span className="micro">{t("MAKE IT YOURS")}</span><h2>{t("A few notes can change everything.")}</h2></div><button className="small-button" onClick={editText}>{(textMode?t('Back to grid'):t('Edit as text'))}</button></div>
    <div className="editor-toolbar"><button className="small-button" aria-label={t("Undo edit")} disabled={!canUndo} onClick={undo}>{t("↶ Undo")}</button><button className="small-button" aria-label={t("Redo edit")} disabled={!canRedo} onClick={redo}>{t("↷ Redo")}</button>
      ...
      <label>{t("Timbre")}<select aria-label={t("{v0} timbre",{v0:t(ROLE_NAMES[role])})} value={song.intent?.[role] ?? ({ lead: 'soft', chord: 'plucked', bass: 'round', perc: 'tight' }[role])} onChange={e => onEdit({ ...song, intent: { ...song.intent, [role]: e.target.value } })}>{Object.keys(INTENTS[role]).map(word => <option key={word} value={word}>{t(word)}</option>)}</select></label>
    </div>
    {textMode ? <div className="text-editor"><label htmlFor="raw-notes">{t(ROLE_NAMES[role])}{t(" · one token per step, . holds, = cuts")}</label><textarea id="raw-notes" value={raw} onChange={e => setRaw(e.target.value)} spellCheck={false} rows={4} /><div><button className="small-button dark" onClick={apply}>{t("Apply notes")}</button><span className="field-error" role="status">{errorText(issue)}</span></div></div> : <>
      <div className="grid-scroll"><div className={`piano-grid ${role}`} role="group" aria-label={t("{v0} note grid",{v0:t(ROLE_NAMES[role])})} style={{ gridTemplateColumns: `44px repeat(${columns}, minmax(38px, 1fr))` }}>
        {[...notes].reverse().map(note => <div className="piano-row" key={note}><button className="pitch-label" onClick={() => onPreview(role, note)} aria-label={t("Preview {v0}",{v0:t(note)})}>{(role === 'perc'?t(({ K: 'Kick', S: 'Snare', H: 'Hat', O: 'Open' }[note])):t(note))}</button>{Array.from({ length: columns }, (_, offset) => {
          const index = page * pageSteps + offset; const active = tokens(pattern[role])[index] === note;
          return <button key={index} className={`grid-cell ${active ? 'filled' : ''} ${offset % stepsPerBeat === 0 ? 'beat-start' : ''}`} aria-label={t("{v0} step {v1} {v2}",{v0:t(ROLE_NAMES[role]),v1:index + 1,v2:t(note)})} aria-pressed={active}

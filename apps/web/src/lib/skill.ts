...
For live updates, GET /api/v1/generations/{id}/events with the same authentication: SSE progress events contain status, elapsed-time timestamps, output-character count and measured render progress, never private reasoning. Reconnect to the same URL after its roughly 20-second stream closes; do not resubmit the prompt. On a terminal event, GET /api/v1/generations/{id} for the complete project/audio references. Ordinary polling at the returned Retry-After interval also remains supported. Queued/composing/validating/saving/rendering are pending; ready/failed/cancelled are terminal. Reuse the same request key after uncertain transport failures; a failed request does not silently call the model again. DELETE cancels unfinished work. On ready, use project.url and render.wavUrl/mp3Url; attach downloaded bytes to a chat as described above. A private URL alone will not play for another user. To share a completed private song, PATCH /api/v1/projects/{projectId} with {"visibility":"public"} or {"visibility":"unlisted"}. The project ID, creator and existing audio stay unchanged. Public sharing always needs the user’s authorization.

import type { AgentGrant } from "./agents";
...
export async function identify(request: Request): Promise<Caller> {
  ...
  if (
    ...
    !presented?.startsWith("cv_agent_")
  )
    return ANONYMOUS;
  if (
    !header &&
  ...
  const client = await db(),
    now = Date.now();
  if (presented?.startsWith("cv_agent_")) {
    const r = await client.execute({
      sql: "select * from agent_grants where hash=? and revoked_at is null and expires_at>?",
    ...
    if (Number(row.last_used ?? 0) < now - 60000)
      void client
        .execute({
          sql: "update agent_grants set last_used=? where id=?",
          args: [now, row.id],
        })
        .catch(() => {});
    return {
  ...
  if (presented) {
    const result = await client.execute({
      sql: `select keys.id,keys.user_id,keys.last_used,users.email from keys join users on users.id=keys.user_id where keys.hash=? and keys.revoked_at is null`,
    ...
    if (Number(row.last_used ?? 0) < now - 60_000)
      void client
        .execute({
          sql: `update keys set last_used=? where id=?`,
          args: [now, row.id],
        })
        .catch(() => {});
    return {
  ...
  const result = await client.execute({
    sql: `select sessions.user_id,users.email from sessions join users on users.id=sessions.user_id where sessions.hash=? and sessions.revoked_at is null and sessions.expires_at>?`,
    args: [await hashKey(token!), now],
  });
  const row = result.rows[0];
  return row
    ? { userId: String(row.user_id), keyId: null, email: String(row.email) }
    : ANONYMOUS;
}
...
async function userFor(email: string): Promise<string> {
  const result = await (
    ...
    sql: `insert into users (id,email,created_at) values (?,?,?) on conflict(email) do update set email=excluded.email returning id`,
...
export async function createKey(email: string, label: string | null) {
  const client = await db(),
    userId = await userFor(email),
    key = PREFIX + secret(),
    id = newId();
  await client.execute({
    sql: `insert into keys (id,user_id,hash,email,label,created_at) values (?,?,?,?,?,?)`,
    args: [
      id,
      userId,
      await hashKey(key),
...
async function loginToken(userId: string) {
  ...
  await client.batch(
    [
      {
        sql: `delete from login_tokens where created_at<?`,
        args: [now - MAGIC_TTL_MS],
      },
      { sql: `delete from sessions where expires_at<?`, args: [now] },
      {
        sql: `insert into login_tokens (hash,user_id,created_at) values (?,?,?)`,
...
export async function createMagicLink(keyId: string): Promise<string> {
  const result = await (
    ...
    sql: `select user_id from keys where id=? and revoked_at is null`,
    args: [keyId],
  });
...
export async function redeemMagicLink(token: string): Promise<string | null> {
  ...
  const results = await client.batch(
    [
      {
        sql: `update login_tokens set used_at=?,session_hash=? where hash=? and used_at is null and created_at>=?`,
        args: [now, sessionHash, tokenHash, now - MAGIC_TTL_MS],
      },
      {
        sql: `insert into sessions (hash,user_id,created_at,expires_at) select ?,user_id,?,? from login_tokens where hash=? and session_hash=? returning user_id`,
        args: [sessionHash, now, now + SESSION_TTL_MS, tokenHash, sessionHash],
      },
      // Preserve consumed legacy links if the deployment is rolled back.
      {
        sql: `update magic set used_at=? where token=? and exists(select 1 from login_tokens where hash=? and session_hash=?)`,
...
export async function revokeSession(request: Request) {
  ...
  if (token)
    await (
      ...
      sql: `update sessions set revoked_at=? where hash=?`,
...
export async function listKeys(userId: string) {
  return (
    await (
      ...
      sql: `select id,label,created_at,last_used,revoked_at from keys where user_id=? order by created_at desc limit 100`,
      args: [userId],
    })
  ).rows;
}
export async function revokeKey(userId: string, id: string) {
  const result = await (
    ...
    sql: `update keys set revoked_at=coalesce(revoked_at,?) where user_id=? and id=? returning id`,

...
export function PlayButton({playing, loading = false, shortcut = false, pause = false, ...props}: ButtonHTMLAttributes<HTMLButtonElement> & {playing: boolean; loading?: boolean; shortcut?: boolean; pause?: boolean}) {
 const t = useT();
  return <button {...props} className={`play-button ${playing ? 'playing' : ''} ${props.className ?? ''}`} aria-label={props['aria-label'] ?? (playing?t((pause ? 'Pause' : 'Stop')):t('Play'))} aria-busy={loading || undefined}><span aria-hidden="true">{(playing?t((pause ? 'Ⅱ' : '■')):t('▶'))}</span>{(playing?t((pause ? 'Pause' : 'Stop')):t('Play'))}{shortcut && <kbd>{t("space")}</kbd>}</button>;
}

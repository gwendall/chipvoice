  ...
  return <div className="range-control" aria-disabled={disabled || undefined}>
    ...
    <div className="range-control-fields">
      <div className="range-control-track">
        <input id={`${id}-slider`} type="range" aria-label={`${label} slider`} aria-valuetext={`${value} ${unit}`} min={min} max={max} step={1} value={value} disabled={disabled}
      ...
      <div className="range-control-number"><input id={id} type="number" aria-label={label} min={min} max={max} step={1} value={draft} disabled={disabled}
        onFocus={event => { begin(); event.target.select(); }} onBlur={commit} onKeyDown={event => { if (event.key === 'Enter') event.currentTarget.blur(); }}
        onChange={event => { const text = event.target.value; setDraft(text); const next = Number(text); if (text.trim() !== '' && Number.isInteger(next) && next >= min && next <= max) change(next); }}/><span>{unit}</span></div>
    </div>
  </div>;
}

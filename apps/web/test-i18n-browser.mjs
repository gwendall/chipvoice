...
try{
 ...
 const song={title:'星のメロディー',chip:'dmg',bpm:144,order:[0],patterns:[{lead:'C4 . E4 .',chord:'C3 . . .',bass:'C2 . . .',perc:'K . H .',chordShape:[[0,4,7]]}]};
 ...
 await page.goto(base+'/ja/s/'+id);await page.getByLabel('言語',{exact:true}).waitFor();assert.equal(await page.title(),song.title);assert.match(await page.locator('meta[name="description"]').getAttribute('content'),/ゲームボーイ/);assert.match(await page.locator('meta[property="og:image"]').getAttribute('content'),new RegExp(`/ja/s/${id}/card`));
...
}catch(error){await page.screenshot({path:new URL('failure.png',out).pathname,fullPage:true}).catch(()=>{});throw error;}
